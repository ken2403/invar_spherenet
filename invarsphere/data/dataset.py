...
class List2GraphDataset(BaseGraphDataset):
    """Convert a list of atoms object into a graph dataset.
...
class Files2GraphDataset(BaseGraphDataset):
    """Convert a directory of atoms files into a graph dataset.
    ...
    def __init__(
        ...
        self.atoms_directory = atoms_directory
        self.subtract_center_of_mass = subtract_center_of_mass
        self.max_n_neighbor_basis = max_n_neighbor_basis
        self.basis_cutoff = basis_cutoff
        self.remove_batch_key = remove_batch_key
        if preprocess:
            self._preprocess(atoms_directory)
        self.inmemory = inmemory
        if inmemory:
            self._data_list = [None for _ in range(self.len())]

    def _preprocess(self, atoms_directory: pathlib.Path):
